...
v = 20
g = 9.81
print("{:.4f}".format(pi * (2 * g * v * h + v ** 3) ** 2 / (4 * g ** 3)))
...
for i in range(Q):
    ...
    x, y = 3 * x + 8 * y, x + 3 * y
ls.sort()
ans = sum(v[1] - 1 for v in ls[1:Q + 1])
print(ans)
'''
可以知道，M(n)=n^2+2n。
如果M(n)为三角数，设存在m，使得n^2+2n=m(m+1)/2。
经过转化后，化为(2m+1)^2-8(n+1)^2=-7。
令x=2m+1,y=n+1，那么每一对关于方程x^2-8y^2=-7的解(x, y)都在原方程都存在着一对解(n, m)。
方程x^2-8y^2=-7有两对初始解(1, 1)和(5, 2)。
可以知道，x_n=3x_{n-1}+8y_{n-1}，y_n=(x_{n-1}+3y_{n-1})。
由上述两对初始解即可根据这个递推生成所有解。
'''
```
Q323
```
