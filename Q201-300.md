...
typedef long long ll;
using namespace std;
const int N=40000000-1,M=25;
int f[N+2],phi[N+2],pr[N/10],v[N+2];
int m=0;
int main(){
    phi[1]=f[1]=1;
    for(int i=2;i<=N;i++){
...
int main(){
    ...
    for(int i=1,p=1;i<=N;i++,p^=1){
        int x=qpow(i,i,M);
        for(int j=0;j<M;j++)
            f[p][j]=(f[p^1][j]+f[p^1][(j-x+M)%M])%mod;
    }
    printf("%lld\n",(f[N&1][0]-1+mod)%mod);
}
```
